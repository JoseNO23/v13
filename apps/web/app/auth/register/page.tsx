...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!respuesta.ok) {
        throw new Error(data?.message ?? data?.mensaje ?? 'No se pudo registrar.');
      }

      setMensaje(data?.mensaje ?? 'Registro exitoso. Revisa la consola del backend.');
      setEmail('');
      setPassword('');
    } catch (err) {
  ...
  return (
    <section style={{ width: '100%', maxWidth: '420px', background: '#1e293b', padding: '2rem', borderRadius: '16px' }}>
      ...
      <p style={{ marginBottom: '1.5rem' }}>
        Crea tu cuenta con correo y contraseña. El enlace de verificación se imprime en la consola del backend.
      </p>
      <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1rem' }}>
        ...
        <button
          ...
          style={{
            ...
            border: 'none',
            fontWeight: 600,
            background: cargando ? '#475569' : '#22c55e',
            color: '#0f172a',
